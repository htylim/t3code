...
export interface ProviderServiceLiveOptions {
  ...
  /**
   * Overrides MCP credential issuance. Tests use this to observe the runtime
...
const makeProviderService = Effect.fn("makeProviderService")(function* (
  ...
  // An unreadable setting denies browser access but does not remove the fork's
  // thread-control tools from the provider session.
    ...
    return yield* Effect.gen(function* () {
      ...
      const resumed = yield* adapter
        .startSession({
          ...
          runtimeMode: input.binding.runtimeMode ?? "auto",
        })
  ...
  const sendTurn: ProviderServiceMethod<"sendTurn"> = Effect.fn("sendTurn")(function* (rawInput) {
    ...
    const inputTextWithSideChatContext =
      parsed.sideChatContext === undefined
        ...
        : [formatSideChatContext(parsed.sideChatContext.mainThreadId), inputTextWithCitations]
