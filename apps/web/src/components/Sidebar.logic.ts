...
type SettledTimestampInput = Pick<SidebarThreadSummary, "createdAt" | "latestUserMessageAt">;

/** Keep settled rows anchored to their last user message. Settling changes
    lifecycle metadata, not the age of the conversation. Threads without a
    user message fall back to creation time. */
export function resolveSettledTimestamp(thread: SettledTimestampInput): string | null {
  return firstValidTimestamp(thread.latestUserMessageAt) ?? firstValidTimestamp(thread.createdAt);
}

// Settlement must not make an old conversation appear recent.
