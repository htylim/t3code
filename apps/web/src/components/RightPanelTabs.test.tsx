...
function renderTabs(
  ...
  options: {
    ...
    empty?: boolean;
    audio?: { audible?: boolean; audioMuted?: boolean };
    previewRuntimeTabId?: ((tabId: string) => string) | null;
  } = {},
) {
  ...
  return renderToStaticMarkup(
    <RightPanelTabs
      ...
      desktopByTabId={{
        "tab-1": overlay(first, options.audio),
...
describe("RightPanelTabs preview favicon", () => {
  it("only offers Chat when the panel has an owning thread", () => {
    expect(renderTabs(null, undefined, { empty: true })).not.toContain(">Chat<");
    expect(renderTabs(null, undefined, { onAddChat: () => undefined, empty: true })).toContain(
      ">Chat<",
    );
  });
...
describe("RightPanelTabs audio indicator", () => {
  ...
  it.each(cases)("audible=$audible muted=$audioMuted", ({ audible, audioMuted, label }) => {
    const html = renderTabs(null, undefined, { audio: { audible, audioMuted } });
    if (label === null) {
  ...
  it("hides the toggle when no runtime tab id can be resolved", () => {
    const html = renderTabs(null, undefined, {
      audio: { audible: true },
      previewRuntimeTabId: null,
    });
    expect(html).not.toContain("Mute Local site");
  });
});
