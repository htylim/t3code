...
// v14 reconciles fork chat persistence with upstream device and PR surface validation.
...
const singletonSurface = (
  kind: Exclude<RightPanelKind, "chat" | "file" | "preview" | "terminal" | "pull-request">,
): RightPanelSurface => {
  switch (kind) {
    case "diff":
      return { id: "diff", kind };
  ...
  persist(
    (set, get) => ({
      ...
      openChat: (owner, target, options) => {
        ...
        set((state) =>
          userAction(state, scopedThreadKey(owner), (current) => {
